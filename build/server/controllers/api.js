 ...
 */
exports.getFoursquare = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const token = yield req.user.tokens.find((token) => token.kind === 'foursquare');
    let trendingVenues;
    let venueDetail;
    let userCheckins;
    axios.all([
 ...
 * GitHub API Example.
 */
exports.getGithub = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const github = new Octokit();
    try {
        const { data: repo } = yield github.repos.get({ owner: 'sahat', repo: 'hackathon-starter' });
 ...
 * Last.fm API example.
 */
exports.getLastfm = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        const { artist: artistInfo } = yield getArtistInfo();
        const topTracks = yield getArtistTopTracks();
        const topAlbums = yield getArtistTopAlbums();
        const artist = {
 ...
 * Twitter API example.
 */
exports.getTwitter = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        const { statuses: tweets } = yield T.get('search/tweets', {
 ...
 * Post a tweet.
 */
exports.postTwitter = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        yield T.post('statuses/update', { status: req.body.tweet });
 ...
 * Steam API example.
 */
exports.getSteam = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        const playerstats = yield getPlayerAchievements();
        const playerSummaries = yield getPlayerSummaries();
        const ownedGames = yield getOwnedGames();
 ...
 * Get /api/twitch
 */
exports.getTwitch = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const token = req.user.tokens.find((token) => token.kind === 'twitch');
    const twitchID = req.user.twitch;
    const twitchClientID = process.env.TWITCH_CLIENT_ID;
    const getUser = (userID) => axios.get(`https://api.twitch.tv/helix/users?id=${userID}`, { headers: { Authorization: `Bearer ${token.accessToken}`, 'Client-ID': twitchClientID } })
        .then(({ data }) => data)
        .catch((err) => Promise.reject(new Error(`There was an error while getting user data ${err}`)));
    const getFollowers = () => axios.get(`https://api.twitch.tv/helix/users/follows?to_id=${twitchID}`, { headers: { Authorization: `Bearer ${token.accessToken}`, 'Client-ID': twitchClientID } })
        .then(({ data }) => data)
        .catch((err) => Promise.reject(new Error(`There was an error while getting followers ${err}`)));
    try {
        const yourTwitchUser = yield getUser(twitchID);
        const otherTwitchUser = yield getUser(44322889);
        const twitchFollowers = yield getFollowers();
 ...
 * Chart example.
 */
exports.getChart = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
 ...
 */
exports.getInstagram = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const token = req.user.tokens.find((token) => token.kind === 'instagram');
    ig.use({ client_id: process.env.INSTAGRAM_ID, client_secret: process.env.INSTAGRAM_SECRET });
    ig.use({ access_token: token.accessToken });
    try {
        const userSelfMediaRecentAsync = exports.promisify(ig.user_self_media_recent);
        const myRecentMedia = yield userSelfMediaRecentAsync();
 ...
 * Lob API example.
 */
exports.getLob = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    let recipientName;
    if (req.user) {
    ...
    try {
        const uspsLetter = yield createAndMailLetter();
        const zipDetails = yield lookupZip();
        res.render('api/lob', {
            title: 'Lob API',
            zipDetails,
            uspsLetter,
